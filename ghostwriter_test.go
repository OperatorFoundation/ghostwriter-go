...
func TestGenerateString(t *testing.T) {
	correct := "abz"
	template := Template{"a$1z"}
	details := []Detail{DetailString{"b"}}

	result, generateError := Generate(&template, details)
	if generateError != nil {
		fmt.Println("failed to generate result")
		t.Fail()
	}

	if *result != correct {
		fmt.Printf("result(%s) does not equal expected(%s)", *result, correct)
		t.Fail()
	}
}
// import XCTest
